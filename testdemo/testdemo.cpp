...
BOOL CtestdemoApp::InitInstance()
{
	...
	// TODO: 应适当修改该字符串，
	// 例如修改为公司或组织名
	SetRegistryKey(_T("testdemo"));
	CRegistryUtil reg(HKEY_CURRENT_USER);
	reg.SetDWordValue(HKEY_CURRENT_USER, REG_CONFIG_SUBKEY, REG_CONFIG_BKTYPE, BKTYPE_COLOR);
	reg.SetDWordValue(HKEY_CURRENT_USER, REG_CONFIG_SUBKEY, REG_CONFIG_BKCOLOR, 3487029);


	// 初始化DuiVision界面库,可以指定语言,dwLangID为0表示自动判断当前语言
	// 11160是应用程序ID,每个DUI应用程序应该使用不同的ID,ID主要用于进程间通信传递命令行时候区分应用
	DWORD dwLangID = 0;
	new DuiSystem(m_hInstance, dwLangID, _T("testdemo.ui"), 11160, IDD_DUIVISIONAPP_DIALOG, _T(""));
